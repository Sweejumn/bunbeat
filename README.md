...
- 🥁 **节拍器**：浏览器端 Web Audio 实现，**逐拍点击分析出的真实拍点**（节拍地图，整首歌零漂移，不再依赖固定间隔），自动对齐歌曲节拍；带相位微调滑杆（±50% 拍距，手动校准，偏好本地保存）与音量调节
...
6. 播放器中可开关节拍器、调节拍器音量、拖动进度、切换上下首；节拍器会逐拍对齐当前歌曲的真实拍点（分析时记录整首歌的每个节拍，变速后换算到处理后音频时间轴），播放到歌曲末尾也不会漂移；若自动对齐仍有偏差，可用相位微调滑杆整体平移点击

## 音频处理依赖

| 用途 | 工具 | 说明 |
|---|---|---|
| BPM / 节拍检测 | librosa | 节拍追踪 + 自相关精修（误差约 ±0.5%），并锁定检测节奏生成全曲节拍地图供节拍器逐拍对齐 |
